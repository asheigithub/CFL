	...
	namespace content
	{
		...
		HuffmanFile huffmanCompress(const char* bytes, size_t len)
		{
			...
			for (size_t i = 0; i < 256 && prioritylist[i] != nullptr; i++)
			{
				...
				std::cout << "'" << prioritylist[i]->bytevalue << "' weight: " << prioritylist[i]->weight << std::endl;
			}

			//移除null节点
			prioritylist.resize(temppriority.size());
			HuffmanNode* root = nullptr;
			...
			for (size_t pos = 0; pos < prioritylist.size(); pos++)
			{
				auto node = prioritylist[pos];
				...
				if (pos < prioritylist.size() - 1)
				{
					canonialCode = (canonialCode + 1) << ((prioritylist[pos + 1])->depth - node->depth);
				}
		...
		UnHuffmanData unHuffmanCompress(const char* bytes, size_t len)
		{
			...
			//std::vector<DecodeSymbolTable*> decodetable;
			...
			for (size_t i = 0; i < header.encodeTableRows; i++)
			{
				...
				{
					dhuffmancode = dhuffmancode << (encoderows[i + 1]->bitSize - row->bitSize);
				}
			}
			...
			if (header.uncompresslen >0)
			{
				...
				int k = encoderows[header.encodeTableRows - 1]->bitSize;
				if (k > 12)
				{
					k = 12;
				}
				...
				for (size_t i = 0; i < ftable_size; i++)
				{
					...
					while (true)
					{
						...
						if (Num >= row->symbolCount)
						{
							...
							if (j == header.encodeTableRows || row->bitSize > k)
							{
								break;
							}
						}
				...
				size_t readstbit = 0;
				for (size_t upos = 0; upos < header.uncompresslen; upos++)
				{
					...
					//先从表里查询
					auto fidx = readBits(pos, k, readstbit);
					...
					while (true)
					{
						...
						unsigned int readdata = (_rb << toreadbits) | readBits(pos, toreadbits, readstbit);
